...
use swarm_wasm_sandbox::{
    CachedNativeModule, CompiledModuleCache, HostCallBudget, ModuleCacheKey, SandboxRuntime,
    wasmtime_version,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tick_sub = client
        .queue_subscribe("swarm.tick.*.player.*".to_string(), "sandbox-workers".to_string())
        .await?;
...
async fn execute_request(
    ...
    let Some(module) = state.modules.get(&module_hash).cloned() else {
        return tick_reply(request, Vec::new(), SandboxExecutionMetrics::default(), "ModuleNotFound");
    };

    let compiled = match state
        .runtime
        .compile_from_cached_native(&module.cached_native, &module.wasm_bytes)
    {
    ...
    let output = match state
        .runtime
        .execute_tick(&compiled, request.snapshot_json.as_bytes())
    {
        Ok(output) => output,
        Err(error) => {
    ...
    while let Some(message) = sub.next().await {
        ...
        let ack = DeployAck {
            ...
            status: if request.wasmtime_version == wasmtime_version() {
                ...
                format!("cached_version_mismatch:{}", request.validation_policy_version)
            },
        };
...
fn tick_reply(
    ...
    let _ = (request.fuel_budget, request.collect_timeout_ms);
    SandboxTickReply {
        tick: request.tick,
